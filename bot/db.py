...
class Database:
        ...
        async with aiosqlite.connect(self.path) as db:
            await db.executescript(
                ...
                CREATE TABLE IF NOT EXISTS order_workers (
                    ...
                    recommended_by_dispatcher INTEGER DEFAULT 0,
                    rating_from_dispatcher INTEGER,
    ...
    async def get_order_workers(self, order_id: int):
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute(
                """
                ...
                JOIN users u ON u.user_id = ow.worker_id
                WHERE ow.order_id=?
                """,
                (order_id,),
            )
    ...
    async def complete_order(self, order_id: int, dispatcher_id: int) -> bool:
        async with aiosqlite.connect(self.path) as db:
            cur = await db.execute("SELECT status, dispatcher_id FROM orders WHERE id=?", (order_id,))
            row = await cur.fetchone()
            if not row:
                return False
            if row[1] != dispatcher_id:
                return False
            if row[0] == "completed":
                return False
    ...
    async def recommend_worker(self, order_id: int, dispatcher_id: int, worker_id: int, rating: int) -> bool:
        async with aiosqlite.connect(self.path) as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute("SELECT dispatcher_id, status FROM orders WHERE id=?", (order_id,))
            order = await cur.fetchone()
            if not order or order["dispatcher_id"] != dispatcher_id or order["status"] != "completed":
                return False

            cur2 = await db.execute(
                "SELECT id, recommended_by_dispatcher FROM order_workers WHERE order_id=? AND worker_id=?",
                (order_id, worker_id),
            )
            rel = await cur2.fetchone()
            if not rel or rel["recommended_by_dispatcher"]:
                return False

            await db.execute(
                "UPDATE order_workers SET recommended_by_dispatcher=1, rating_from_dispatcher=? WHERE id=?",
                (rating, rel["id"]),
            )
            await db.execute("UPDATE users SET recommends = recommends + 1 WHERE user_id=?", (worker_id,))
            cur3 = await db.execute(
                "SELECT AVG(rating_from_dispatcher) FROM order_workers WHERE worker_id=? AND rating_from_dispatcher IS NOT NULL",
                (worker_id,),
            )
            avg_row = await cur3.fetchone()
            avg = float(avg_row[0]) if avg_row and avg_row[0] is not None else 5.0
            await db.execute("UPDATE users SET rating=? WHERE user_id=?", (round(avg, 2), worker_id))
            await db.commit()
            return True

    async def list_priority_workers(self) -> list[int]:
        now = datetime.utcnow().isoformat()
        async with aiosqlite.connect(self.path) as db:
            cur = await db.execute(
                """
                ...
                FROM users
                WHERE role='worker'
                ORDER BY CASE WHEN premium_until > ? THEN 0 ELSE 1 END, registered_at ASC
                """,
                (now,),
            )
            rows = await cur.fetchall()
