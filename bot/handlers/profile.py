...
from bot.keyboards import profile_role_kb
...
async def set_role(callback: CallbackQuery, db: Database):
    role = callback.data.split(":", 1)[1]
    await db.set_role(callback.from_user.id, role)
    await callback.answer("Роль обновлена")
    await callback.message.answer(
        f"✅ Ваша новая роль: {'👷 Грузчик' if role == 'worker' else '👨‍💼 Диспетчер'}"
    )
